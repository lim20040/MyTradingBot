...
async def get_combined_report(period_type, current_seed):
    ...
    # 04-23 이전 데이터는 수학적으로 Max 함수를 사용하여 필터링
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(url, headers=headers) as response:
            ...
            if trades_count == 0:
                return f"📊 {actual_start_dt.strftime('%Y-%m-%d')} 이후 매매 내역이 없습니다."
            ...
            # 리포트 구성 (6가지 항목)
            report = f"📊 **성과 리포트 (KST)**\n"
            report += f"📅 {actual_start_dt.strftime('%Y-%m-%d %H:%M')} ~ {now_kst.strftime('%m-%d %H:%M')}\n"
            report += "━━━━━━━━━━━━━━━━━━\n\n"
            ...
            report += f"4️⃣ **수익률(ROI):** {roi:+.2f}%\n"
            report += f"5️⃣ **평균 익/손:** {avg_win:+.2f} / {avg_loss:.2f} USDT\n"
            report += f"6️⃣ **최대 익/손:** {max_win:+.2f} / {max_loss:.2f} USDT"
