...
async def get_combined_report(days):
    method = "GET"
    # 포지션 히스토리 경로 (화면에 보이는 Realized PnL 수치를 그대로 가져오는 경로)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(url, headers=headers) as response:
            ...
            # API 인증 실패 시 에러 메시지 출력
            if res.get("code") != "0":
                return f"❌ **OKX API 에러**: {res.get('msg')}\n(키, 시크릿, 패스프레이즈를 다시 확인해주세요!)"
            ...
            for item in data:
                ...
                # 'realizedPnl' 필드를 우선적으로 가져와서 화면 수치와 맞춥니다.
